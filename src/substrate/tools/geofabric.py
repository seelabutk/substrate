...
class GeoFabric(Tool):  # pylint: disable=too-many-instance-attributes
	def __init__(self, config, data_sources):
		...
		fallback_dir = os.path.join(os.path.dirname(__file__), 'geofabric')
		print(self.config)
		self.geofabric_path = self.config['geofabric'].get('directory', [fallback_dir])[0]  # noqa: E501
		self.app_path = os.path.join(self.geofabric_path, 'app')
		self.config_path = os.path.join(self.geofabric_path, 'config')
